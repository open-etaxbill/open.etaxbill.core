...
namespace OpenTax.Engine.Library.Data.MSSQL
{
    public class MsAsyncDataSet
    {
        ...
        //
        //-----------------------------------------------------------------------------------------------------------------------------
        public MsAsyncDataSet(string p_filepath, string p_filename, int p_rowsperpage, string p_extension)
        {
            if (p_rowsperpage > 0)
                RowsPerPage = p_rowsperpage;

            if (String.IsNullOrEmpty(p_filepath) == true)
                p_filepath = Path.Combine(CfgHelper.SNG.GetWorkingFolder("AsyncDS"), c_rootpath);
        ...
        private OpenTax.Engine.Library.Data.MSSQL.MsDataHelper DatHelper
        {
            get
            {
        ...
        //
        //-----------------------------------------------------------------------------------------------------------------------------
        private string _topCommandTextBuild(string p_text)
        {
            var _result = "";
            var _tops = 0;
        ...
        private string _recordCountCommandTextBuild(string p_text)
        {
            var _result = "";
            var _norec = 0;
        ...
        //
        //-----------------------------------------------------------------------------------------------------------------------------
        private void _fillBinarySet(string p_connection_string, string p_cmdtxt, MsDatParameters p_dbps)
        {
            DatStore.Merge(DatHelper.SelectDataSet(p_connection_string, p_cmdtxt, p_dbps));
        }
        ...
        private void _readBinarySet(int p_page_number, out int p_no_reccord)
        {
            p_no_reccord = 0;
            ...
            if (DatStore.Tables.Count > 0)
            {
                ...
                p_no_reccord = (int)_rfs.Length / _reclen;

                var _lastpage = (int)((_rfs.Length - 1) / (long)(_reclen * RowsPerPage) + 1);
                if (p_page_number < 1)
                    p_page_number = 1;
                else if (p_page_number > _lastpage)
                    p_page_number = _lastpage;

                _rfs.Seek((p_page_number - 1) * _reclen * RowsPerPage, SeekOrigin.Begin);
        ...
        //
        //-----------------------------------------------------------------------------------------------------------------------------
        public DataSet SelectTopSet(string p_connection_string, string p_cmdtxt, MsDatParameters p_dbps)
        {
            _fillBinarySet(p_connection_string, _topCommandTextBuild(p_cmdtxt), p_dbps);
            return DatStore;
        }

        public int RecordCount(string p_connection_string, string p_cmdtxt, MsDatParameters p_dbps)
        {
            _fillBinarySet(p_connection_string, _recordCountCommandTextBuild(p_cmdtxt), p_dbps);
            return (int)DatStore.Tables[0].Rows[0][0];
        }

        public DataSet WriteBinary(string p_connection_string, string p_cmdtxt, MsDatParameters p_dbps, out int p_no_reccord)
        {
            _cleanBinarySet();
            _fillBinarySet(p_connection_string, p_cmdtxt, p_dbps);

            p_no_reccord = 0;
            if (_writeBinarySet() == true)
                _readBinarySet(1, out p_no_reccord);

            return DatStore;
        }

        public DataSet ReadBinary(int p_page_number, out int p_no_reccord)
        {
            _readBinarySet(p_page_number, out p_no_reccord);
            return DatStore;
        }
