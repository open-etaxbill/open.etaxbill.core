...
namespace OdinSdk.eTaxBill.Security.Signature
{
    ...
    public class SeedCs
    {
        ...
        /// <returns></returns>
        public byte[] Encrypt(byte[] plain_data, byte[] seed_key)
        {
            return Encrypt(plain_data, seed_key, false);
        }
        ...
        /// <param name="plain_data"></param>
        /// <param name="seed_key"></param>
        /// <param name="cbc_padding"></param>
        /// <returns></returns>
        public byte[] Encrypt(byte[] plain_data, byte[] seed_key, bool cbc_padding)
        {
            var _insize = plain_data.Length;

            int _outsize;
            if (cbc_padding == true)
                _outsize = (int)((plain_data.Length + 16) / 16) * 16;
            else
                _outsize = (int)((plain_data.Length) / 16) * 16;

            byte[] _result = new byte[_outsize];
            Buffer.BlockCopy(plain_data, 0, _result, 0, _insize);

            if (cbc_padding == true)
            {
                var _padding = _outsize - _insize;
                if (_padding == 0)
                    _padding = 16;
            ...
            SeedEncRoundKey(ref _roundKey, seed_key);
        ...
        /// <param name="seed_key"></param>
        /// <returns></returns>
        public byte[] Decrypt(byte[] encrypted_data, byte[] seed_key)
        {
            return Decrypt(encrypted_data, seed_key, false, new byte[0]);
        }
        ...
        /// <param name="seed_key"></param>
        /// <param name="cbc_padding"></param>
        /// <param name="iv_bytes"></param>
        /// <returns></returns>
        public byte[] Decrypt(byte[] plain_data, byte[] seed_key, bool cbc_padding, byte[] iv_bytes)
        {
            byte[] _result = new byte[plain_data.Length];

            var _outsize = plain_data.Length;
            uint[] _roundKey = new uint[32];
            SeedEncRoundKey(ref _roundKey, seed_key);

            // CBC 모드 초기값 IV
            byte[] _prevData;
            if (iv_bytes != null && iv_bytes.Length > 0)
                _prevData = iv_bytes;
            else
            ...
            {
                byte[] _dataBlock = new byte[16];
                Buffer.BlockCopy(plain_data, i, _dataBlock, 0, 16);
                ...
                Buffer.BlockCopy(_dataBlock, 0, _result, i, 16);
                if (cbc_padding == true)
                    Buffer.BlockCopy(plain_data, i, _prevData, 0, 16);
            }

            if (cbc_padding == true)
            {
                var _padding = _result[_outsize - 1];
                if (_padding > 0 && _padding <= 16)
                {
                    ...
                    {
                        _outsize -= _padding;
                        Buffer.BlockCopy(_result, 0, plain_data, 0, _outsize);

                        _result = new byte[_outsize];
                        Buffer.BlockCopy(plain_data, 0, _result, 0, _outsize);
                    }
                }
            }
